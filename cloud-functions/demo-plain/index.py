...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header h1 { color: #306998; font-size: 32px; margin-bottom: 10px; }
        ...
        .btn-primary { background: #306998; color: white; }
        .btn-primary:hover { background: #255070; transform: translateY(-2px); }
        ...
        .test-icon { width: 32px; height: 32px; border-radius: 8px; display: flex;
                     ...
                     background: linear-gradient(135deg, #306998, #FFD43B); color: white; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>🐍 Python 函数测试</h1>
            <div class="version">Pure Python (no framework) | EdgeOne Pages Cloud Functions</div>
        </div>
    ...
    <script>
        const B = "/demo-plain";
        const tests = [
            { id: 1, name: "基础信息", method: "GET", path: B + "/info", desc: "获取函数基本信息", check: "Plain Python" },
            { id: 2, name: "健康检查", method: "GET", path: B + "/health", desc: "服务健康状态", check: "healthy" },
            { id: 3, name: "获取用户", method: "GET", path: B + "/users/1", desc: "GET 用户信息", check: "user_id" },
            { id: 4, name: "创建用户", method: "POST", path: B + "/users", desc: "POST 创建用户", body: {username: "测试", email: "test@example.com"}, check: "created" },
            { id: 5, name: "搜索功能", method: "GET", path: B + "/search?q=test&limit=5", desc: "查询参数搜索", check: "query" },
            { id: 6, name: "GET 回显", method: "GET", path: B + "/echo?msg=hello", desc: "回显请求信息", check: "method" },
            { id: 7, name: "POST 回显", method: "POST", path: B + "/echo", desc: "回显 POST 请求", body: {data: "test"}, check: "method" },
            { id: 8, name: "请求头", method: "GET", path: B + "/headers", desc: "查看请求头信息", check: "user_agent" },
            { id: 9, name: "当前时间", method: "GET", path: B + "/time", desc: "获取服务端时间", check: "timestamp" },
            { id: 10, name: "JSON 处理", method: "POST", path: B + "/json", desc: "处理 JSON 请求体", body: {name: "test", value: 42}, check: "received" },
            { id: 11, name: "错误处理", method: "GET", path: B + "/error", desc: "触发错误", expectError: true }
        ];
