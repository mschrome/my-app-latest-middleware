...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header h1 { color: #4caf50; font-size: 32px; margin-bottom: 10px; }
        ...
        .test-icon { width: 32px; height: 32px; border-radius: 8px; display: flex;
                     ...
                     background: linear-gradient(135deg, #4caf50, #2196f3); color: white; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎸 Django 框架测试</h1>
            <div class="version">Django 4.0+ | EdgeOne Pages Python Cloud Functions</div>
        </div>
    ...
    <script>
        ...
        const tests = [
            ...
            { id: 3, name: "创建用户", method: "POST", path: B + "/users/create/", desc: "POST 创建用户", body: {username: "测试", email: "test@example.com"}, check: "User created" },
            { id: 4, name: "更新用户", method: "PUT", path: B + "/users/1/update/", desc: "PUT 更新用户", body: {username: "更新", email: "update@example.com"}, check: "updated" },
            { id: 5, name: "删除用户", method: "DELETE", path: B + "/users/1/delete/", desc: "DELETE 删除用户", expectStatus: 204 },
            { id: 6, name: "搜索功能", method: "GET", path: B + "/search/?q=test&category=all", desc: "搜索 API", check: "query" },
            { id: 7, name: "SSE 流", method: "GET", path: B + "/stream/", desc: "Server-Sent Events", stream: true },
            { id: 8, name: "JSON 流", method: "GET", path: B + "/stream/json/", desc: "JSON 流式传输", stream: true },
            { id: 9, name: "大数据流", method: "GET", path: B + "/stream/large/", desc: "大数据流", stream: true },
            { id: 10, name: "请求头回显", method: "GET", path: B + "/headers/echo/", desc: "回显请求头", check: "user_agent" },
            { id: 11, name: "自定义响应头", method: "GET", path: B + "/headers/custom/", desc: "设置自定义头", check: "custom headers" },
            { id: 12, name: "设置 Cookie", method: "GET", path: B + "/cookie/set/", desc: "设置 Cookie", check: "Cookie set" },
            { id: 13, name: "读取 Cookie", method: "GET", path: B + "/cookie/get/", desc: "读取 Cookie", check: "cookie" },
            { id: 14, name: "方法测试", method: "POST", path: B + "/methods/test/", desc: "HTTP 方法", body: {test: "data"}, check: "method" },
            { id: 15, name: "JSON 变体", method: "GET", path: B + "/json/variants/", desc: "不同 JSON 响应", check: "standard" },
            { id: 16, name: "性能测试", method: "GET", path: B + "/performance/compute/1000/", desc: "计算性能", check: "result" }
        ];
